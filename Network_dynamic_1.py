...
class NetworkMPC(Network):
    def __init__(self, reuse=False):
        ...
        self.traffic = np.array([0]*(self.node_number-1))
    ...
    def generate_source(self):
        ...
        for i in range(1, self.node_number):
            ...
            for j in range(num_mpc):
                t = list(self.MPC_connectivity[i]).index(1, j)  # Target node
                s2sh_route = self.find_route(i, t)
                for node in s2sh_route[1:]:
                    self.traffic[node-1] += 2
                sh2sink_route = self.find_route(t, 0)
                for node in sh2sink_route[:-1]:
                    self.traffic[node-1] += 2
            s2sink_route = self.find_route(i, 0)
            self.traffic[i - 1] += 1
    ...
    def get_load(self):
        ...
        self.max_load = np.array(self.max_load)
...
if __name__ == "__main__":
    ...
    # Total load
    total_load = list()
    net.MPC_connectivity.fill(0)
    net.operate()
    total_load.append(np.sum(net.traffic))
    for i in range(14):
        for j in range(1, net.node_number):
            net.act_action(j, 2)
        net.operate()
        total_load.append(np.sum(net.traffic))
    ...
    for i in range(1, 16):
        ...
        net.operate()
        if i==1:
            plt.plot(x, net.traffic, 'g--')
        elif i==15:
            plt.plot(x, net.traffic, 'b--')
        else:
            plt.plot(x, net.traffic, 'r-')
        for j in range(1, net.node_number):
            net.act_action(j, 2)
    ...
    for j in range(10):
        ...
        #c = 1 - abs(net.traffic- np.mean(net.traffic)) / (np.max(net.traffic) - np.min(net.traffic))
        y = a*b*c
        if j==0:
            plt.plot(x, y, 'g--')
        elif j == 9:
            ...
            plt.plot(x, y, 'b-')
        print(np.sum(y)/(net.node_number-1))
        for i in range(1, net.node_number):
            net.act_action(i, 1)
        #print(net.MPC_connectivity)
    '''
