...
class Catch(object):
    ...
    def _update_state(self, action):
        ...
        f0, f1, b0, b1 = state[0]
        newb1 =
        newb1 = min(max(1, b1 + action), self.grid_size - 1)
        f0 += 1
        out = np.asarray([f0, f1, newb0, newb1])
        out = out[np.newaxis]
    ...
    def reset(self):
        ...
        basket_column = np.random.randint(1, self.grid_size - 2, size=1)
        self.state = np.asarray([0, basket_row, basket_column])[np.newaxis]
...
def baseline_model(grid_size,num_actions,hidden_size):
    ...
    Qmodel = Sequential()
    Qmodel.add(Dense(hidden_size, input_shape=(grid_size**2,), activation='relu'))
    Qmodel.add(Dense(hidden_size, activation='relu'))
    Qmodel.add(Dense(num_actions))
    #Qmodel.compile(sgd(lr=.1), "mse")
    sgd = keras.optimizers.SGD(lr=.1)
    Qmodel.compile(loss="mse", optimizer=sgd)
    return Qmodel
...
epoch = 500 # Number of games played in training, I found the model needs about 4,000 games till it plays well
# Train the model
