...
class NetworkMPC(Network):
    def __init__(self, reuse=False):
        ...
        self.action_space = [n for n in range(0, 5)]
    ...
    def update_candidates(self):
        for i in range(1, self.node_number):
            value = []
            for nei in self.node_object[str(i)].neighbor:
                value.append((nei, self.traffic[nei-1]))
            value.sort(key=lambda x:x[1])
            tmp = []
            for k in range(10):
                tmp.append(value[k][0])
            self.node_object[str(i)].neighbor = tmp
    ...
    def generate_source(self):
        ...
        for i in range(1, self.node_number):
            ...
            for j in range(num_mpc):
                ...
                self.traffic[i-1] += 2
                self.traffic[t-1] += 2
                for node in s2sh_route[1:-1]:
                    self.traffic[node-1] += 4
    ...
    def get_reward(self, node_id):
        ...
        reward = np.sum(p*b)/(self.node_number-1)
        #reward = H[node_id-1]*p[node_id-1]*b[node_id-1]
        return reward

    def get_load(self):
        self.naive_traffic = [1]*(self.node_number-1)
        for i in range(1, self.node_number):
            s2sink_route = self.find_route(i, 0)
            for node in s2sink_route[1:-1]:
                self.naive_traffic[node-1] += 2
        print(self.naive_traffic)
...
if __name__ == "__main__":
    ...
    a = [0.2]*(net.node_number-1)
    #plt.plot(x, a, 'g--')
    plt.ylabel('Privacy protection indicator')
    plt.xlabel('Node ID')
    plt.xticks(np.arange(1,net.node_number, 1))

    b = 1 - abs(net.traffic - np.mean(net.traffic)) / np.mean(net.traffic)
    #plt.plot(x, b, 'r--')
    plt.ylabel('Load balancing indicator')
    plt.xlabel('Node ID')
    plt.xticks(np.arange(1,net.node_number, 1))
    ...
    y = a*b
    plt.plot(x, y, 'b--')
    reward = np.sum(a*b)/(net.node_number-1)
    plt.ylabel('Initial reward')
    plt.xlabel('Node ID')
    plt.xticks(np.arange(1,net.node_number, 1))
    print(reward)
    '''
